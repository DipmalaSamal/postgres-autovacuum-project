...
try:
    ...
    for _ in range(100000):  # Adjust for amount of data
        cur.execute('''
...
try:
    ...
    for _ in range(100000):  # Number of rows to generate dead tuples
        # Insert rows
        cur.execute('''
            INSERT INTO test_schema.test_table (name, address, email) 
            VALUES (%s, %s, %s);
        ...
        cur.execute('''
            ...
            WHERE id IN (SELECT id FROM test_schema.test_table ORDER BY random() LIMIT 8000);
        ''')
